...
import { Link, Routes, Route, useMatch, Navigate, useNavigate } from 'react-router-dom'
...
const AnecdoteList = ({ anecdotes }) => (
  <div>
    <h2>Anecdotes</h2>
    <ul>
      {anecdotes.map(anecdote => <Link to={`/anecdote/${anecdote.id}`} key={anecdote.id} style={{ display: "block" }}>{anecdote.content}</Link>)}
    </ul>
  </div>
)
